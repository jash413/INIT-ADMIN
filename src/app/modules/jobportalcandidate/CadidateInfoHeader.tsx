...
const CadidateInfoHeader: FC<CadidateInfoHeaderProps> = ({ id }) => {
  ...
  const handleDownloadResume = async (id: number, fileName: string) => {
    try {
      ...
      // Append link to the body, trigger the download, and remove it
  ...
  const handleDownloadProImg = async (id: number) => {
    try {
      ...
      // Create a URL from the blob and update the profile image state
      if (response.data) {
      ...
      console.error("Error downloading the profile image:", error);
    }
  };
  useEffect(() => {
    if (id) {
          ...
          <div className="card-body">
                  ...
                  {candidate.candidate_edu.map((edu: any, index: number) => (
                    ...
                    >
                      <div className="card p-2 pt-5">
                        <p>
                          <strong>{edu.can_edu}:</strong> {edu.can_scho} (
                          {edu.can_pasy})
                        </p>
                        <p className="text-truncate">
                          Stream: {edu.can_stre}, Percentage: {edu.can_perc},
                          CGPA: {edu.can_cgpa}
                        </p>
                      </div>
                    </div>
                ...
                <div className="row">
                  {candidate.candidate_exp.map((exp: any, index: number) => (
                    ...
                    >
                      <div className="card p-2 pt-5">
                        <p>
                          <strong>{exp.emp_name}:</strong> {exp.exp_desg} (
                          {exp.exp_type})
                        </p>
                        <p>
                          From: {moment(exp.job_stdt).format("MM/DD/YYYY")} To:{" "}
                          {moment(exp.job_endt).format("MM/DD/YYYY")}
                        </p>
                        <p>CTC: {exp.cur_ctc}</p>
                      </div>
                    </div>
            ...
            {candidate?.can_resume && (
              <span
                ...
                onClick={() =>
                  handleDownloadResume(candidate?.can_code, candidate.can_name)
                }
